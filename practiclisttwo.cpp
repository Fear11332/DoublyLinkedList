...
void List::push_back(ValueType data)
{
...
void List::push_front(ValueType data)
{
    Node* ptr = new Node(data);
...
void List::insert(ValueType value, size_t index)
{
...
List::Node* List::findNode(const ValueType& value) const
{
	...
	else if(current2->data == value)
	{
        ...
        while (current->pnext != nullptr || current2->previous!=nullptr) {
            current = current->pnext;
